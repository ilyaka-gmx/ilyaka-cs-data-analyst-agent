...
cp .env.example .env
```
**STOP. Ask the user for their Nebius API key.** Then write it into `.env`:
```bash
# Replace <key> with the actual key the user gave you
echo 'NEBIUS_API_KEY=<key>' >> .env
```

**Step 3 — Fast sanity (no API key needed, ~10s):**
```bash
uv run pytest tests/test_tools.py tests/test_ui.py tests/test_quality.py tests/test_mcp.py -q
```

**Step 4 — Health check (needs the API key from step 2):**
```bash
