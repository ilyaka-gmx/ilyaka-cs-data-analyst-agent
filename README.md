...
The agent classifies queries via a dedicated router, uses 11 typed tools with Pydantic schemas, persists conversations via SQLite, maintains per-user profiles, exposes tools through MCP, and includes a cross-model quality judge. All LLM inference runs on [Nebius Token Factory](https://tokenfactory.nebius.com/).
...
- SSE streaming with live reasoning steps
- Suggestion chips (contextual after each response)
- Admin tab with per-query execution traces
- Memory Insights tab (4-panel dashboard)
...
| `test_agent.py`        | 21    | Full graph integration, persistence, profile  |
...
User Query → Router (Qwen3-30B) → classification
  ├── structured/unstructured/recommend → Agent ReAct Loop (Qwen3-235B) ↔ Tools
...
| `recall_profile`       | Read user profile                              |
...
| **Semantic**   | `profiles/{user_id}.json`      | Distilled user facts: name, role, interests, preferences                           |
...
│   ├── memory.py           # User profile CRUD (thread-safe)
...
├── frontend/
│   └── index.html          # Single-file SPA (primary web UI)
...
This project can be considered as a POC towards real project. If we would continue to real project, following points are coming to the roadmap.
...
- **Semantic search in memory**: integrate embedding-based recall (e.g., Mem0 or LangGraph `BaseStore` with `Qwen3-Embedding-8B`) for more intelligent profile matching.
